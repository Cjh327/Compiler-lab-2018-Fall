...
void parseDecList(Ast decList, Type type, map<string, Symbol>& local_symbol_table) {
    ...
    while (1) {
        ...
        if (sym || t) {
            ...
            Variable var = new Variable_(newVarId(), symbol->name, symbol->u.type);
            var_table.insert(pair<string, Variable>(symbol->name, var));
        }
...
string parseVarDec(Ast varDec, Type type) {
    ...
    while (child->tag != TAG_ID) {
        ...
        type->u.array.size_list.push_back(child->first_sibling->first_sibling->u.ival);    
        type->dim++;
...
Symbol buildFunc(Ast funDec, Type type, SKind kind) {
    ...
    if (sibling->tag == TAG_VAR_LIST) {
        ...
        while (1) {
            ...
            Variable var = new Variable_(newVarId(), arg->name, arg->type, true);
            var_table.insert(pair<string, Variable>(var->name, var));
...
void copyType(Type dest, Type src) {
    ...
    switch (src->kind) {
        ...
        case ARRAY:     dest->u.array.elem = new Type_();
                        ...
                        dest->u.array.size_list = src->u.array.size_list;
                        break;
...
void reportError(int error_type, int line_no, string str) {
    ...
    switch(error_type) {
        ...
        case 8: cout << "Type mismatched for return."; break;
